...
export async function HomeSections({ sections }: { sections: HomeSectionDTO[] }) {
  ...
  return (
    <>
      {resolved.map(({ section: s, products, categories, videos }, idx) => {
        ...
        if (s.type === "banner") {
          ...
          return (
            <section key={s.id} className="container-5xl py-8 md:py-12">
              <Reveal>{inner}</Reveal>
            </section>
          );
        }
        ...
        return (
          <section
            key={s.id}
            className={cn("py-8 md:py-12", alt && "border-y border-border bg-muted/30")}
          >
